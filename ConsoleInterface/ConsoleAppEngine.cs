namespace ConsoleInterface
{
    ...
    class ConsoleAppEngine
    {
                ...
                case SortEnum.BySurName:
                    {
                        ...
                        if (!string.IsNullOrEmpty(temp = Console.ReadLine()))
                        {
                            ...
                            //searchList = EnginePhoneBook.Services.GetPhoneBook.SearchBySurname(temp).t;
                            return;
                        }
                        else
                        {
        ...
        public static Error StartWork()
        {
            int contactStartIndex = 0;
            int contactEndIndex = 0;
            Services.LoadBaseConfig();
            PhoneBook myPhoneBook = new PhoneBook(Services.GetPhoneBook.GetPersonList());
            ...
            while (true)
            {
                ...
                switch (WaitUserCommand())
                {
                    ...
                    case Commands.ClearList:
                        {
                            Services.Clear();
                            myPhoneBook = Services.GetPhoneBook;
        ...
        private static void Add(ref PhoneBook myPhoneBook)
        {
            ...
            Services.Add(surName,foreName,middleName,phoneNumber);
            myPhoneBook = Services.GetPhoneBook;
            myPhoneBook.Sort(1);
        }
